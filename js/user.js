...
class FieldValidator {
  ...
  static async validate(...validator) {
    const proms = validator.map((v) => v.validate());
    const results = await Promise.all(proms); //所有任务完成才会得到[true,false,true]这种格式的返回
    return results.every((r) => r); //数组里面every方法，每一项必须满足条件才会返回true
  }
}
